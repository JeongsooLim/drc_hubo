...
//      member functions
// =======================================
inline double  vec3_::norm      ()                  const {return sqrt(x*x+y*y+z*z);}
...
inline double  vec4_::norm      ()                  const {return sqrt(x*x+y*y+z*z);}
...
// =======================================
// vec3
inline double  norm      (const vec3_& v)                  {return sqrt(v.x*v.x+v.y*v.y+v.z*v.z);}
...
// vec4
inline double  norm      (const vec4_& v)                  {return sqrt(v.x*v.x+v.y*v.y+v.z*v.z);}
...
// quat
inline double  norm      (const quat_& q)                  {return sqrt(q.w*q.w+q.x*q.x+q.y*q.y+q.z*q.z);}
...
inline quat   operator !(const quat_& q)                  {return quat(q.w,-q.x,-q.y,-q.z);}
// pos
inline pos_&  negate    (pos_& v)                         {v.x = -v.x; v.y = -v.y; v.z = -v.z; v.qw=-v.qw; v.qx=-v.qx; v.qy=-v.qy; v.qz=-v.qz; return v;}
